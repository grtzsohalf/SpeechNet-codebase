...
from src.module import VGGExtractor, CNNExtractor, PseudoDownsampler, RNNLayer
...
class AudioEncoder(nn.Module):
    ...
    def __init__(self, process_group, input_size, prenet, module, dim, dropout, 
    ...
    def forward(self, input_x, enc_len):
        ...
        if self.module in ['LSTM', 'GRU']:
            ...
            content_enc_mask = make_non_pad_mask(content_enc_len.tolist(), torch.zeros((content_input_x.shape[0], content_input_x.shape[1]))).to(content_input_x.device).unsqueeze(-2)
            speaker_enc_mask = make_non_pad_mask(speaker_enc_len.tolist(), torch.zeros((speaker_input_x.shape[0], speaker_input_x.shape[1]))).to(speaker_input_x.device).unsqueeze(-2)
            nvtx.range_pop()
            ...
            if self.layer_speaker > 0:
                ...
                pooled_speaker_input_x, pooled_sap_input_enc_mask = self.speaker_aggregater(speaker_input_x, sap_input_enc_mask)
                nvtx.range_pop()
            else:
                speaker_input_x = None
                speaker_enc_mask = None
                
        return content_enc_len, content_input_x, content_enc_mask, speaker_input_x, speaker_enc_mask, pooled_speaker_input_x
