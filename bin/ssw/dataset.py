...
class WaveSplitWhamDataset(data.Dataset):
    ...
    Args:
        ...
        nondefault_nsrc (int, optional): Number of sources in the training
            ...
            separation tasks.
    """
    def __init__(self, json_dir, task, sample_rate=16000, segment=4.0, target_level=-25,
    ...
    def normalize_wav_decibel(self, audio, target_level):
    ...
    def __getitem__(self, idx):
        ...
        x = torch.Tensor(x)
        x = self.normalize_wav_decibel(x, self.target_level)
        ...
        for src in c_ex["sources"]:
            s, _ = sf.read(src, start=rand_start,
                            stop=stop, dtype='float32')
            s = torch.Tensor(s)
            s = self.normalize_wav_decibel(s, self.target_level)
            source_arrays.append(s)
        sources = torch.from_numpy(np.vstack(source_arrays)).unsqueeze(1)
        return x.unsqueeze(1), sources, torch.Tensor(c_ex["spk_id"]).long()
